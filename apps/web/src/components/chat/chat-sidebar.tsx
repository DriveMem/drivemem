...
import { mockConversations, type Conversation } from "@/lib/mock-chat"
  ...
  if (collapsed) {
    return (
      <div className="flex flex-col items-center py-2 border-r border-border w-12">
        <Button variant="ghost" size="icon" onClick={() => setCollapsed(false)} className="h-8 w-8 mb-2">
          <PanelLeft className="h-4 w-4" />
        </Button>
        <Button variant="ghost" size="icon" asChild className="h-8 w-8">
          <Link href="/chat"><Plus className="h-4 w-4" /></Link>
        </Button>
      </div>
    )
  }
