...
export default function LandingPage() {
  return (
    <main className="min-h-screen bg-white text-[#1C1B18] selection:bg-[#4F5BD5]/20">
      ...
      {/* ===== grid bg ===== */}
      <div className="pointer-events-none fixed inset-0 z-0 bg-[linear-gradient(rgba(0,0,0,.03)_1px,transparent_1px),linear-gradient(90deg,rgba(0,0,0,.03)_1px,transparent_1px)] bg-[size:64px_64px]" />
      ...
      <section className="relative z-10 flex min-h-[90vh] flex-col items-center justify-center bg-gradient-to-b from-[#F4F5FD] to-white px-6 text-center">
        <FadeIn>
          <h1 className="mx-auto max-w-4xl text-5xl font-bold leading-tight tracking-tight text-[#1C1B18] sm:text-7xl">
            让 AI 记住你的一切
          </h1>
      ...
      <section className="relative z-10 px-6 py-20">
        <div className="mx-auto max-w-5xl" style={{ perspective: "1200px" }}>
          <div className="rounded-xl border border-[#E5E4E1] bg-white p-1 shadow-2xl shadow-[#4F5BD5]/10 ring-1 ring-[#4F5BD5]/5" style={{ transform: "rotateX(2deg)" }}>
      ...
      <section id="features" className="relative z-10 mx-auto max-w-6xl px-6 py-24">
        <FadeIn>
          <h2 className="text-center text-3xl font-bold text-[#1C1B18] sm:text-4xl">AI 让你的文件更智能</h2>
          <p className="mx-auto mt-4 max-w-2xl text-center text-[#6B6966]">不只是存储，更是理解。AI Drive 让每个文件都成为可交互的知识。</p>
          <div className="mt-16 grid gap-6 sm:grid-cols-3">
            <div className="group rounded-2xl border border-[#E5E4E1] bg-white p-8 transition hover:border-[#4F5BD5]/30 hover:shadow-md">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-[#4F5BD5]/15 to-purple-500/15 mb-4">
                <Brain className="h-6 w-6 text-[#4F5BD5]" />
              </div>
              <h3 className="text-xl font-semibold text-[#1C1B18] mb-2">AI 智能摘要</h3>
              <p className="text-[#6B6966] leading-relaxed">上传文件自动生成摘要，不用阅读全文就能了解核心内容</p>
            </div>
            <div className="group rounded-2xl border border-[#E5E4E1] bg-white p-8 transition hover:border-[#4F5BD5]/30 hover:shadow-md">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-purple-500/15 to-pink-500/15 mb-4">
                <MessageSquare className="h-6 w-6 text-purple-500" />
              </div>
              <h3 className="text-xl font-semibold text-[#1C1B18] mb-2">跨文件问答</h3>
              <p className="text-[#6B6966] leading-relaxed">同时问 AI 多个文件的内容，获得跨文档的综合分析</p>
            </div>
            <div className="group rounded-2xl border border-[#E5E4E1] bg-white p-8 transition hover:border-[#4F5BD5]/30 hover:shadow-md">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-cyan-500/15 to-[#4F5BD5]/15 mb-4">
                <Globe className="h-6 w-6 text-cyan-500" />
              </div>
              <h3 className="text-xl font-semibold text-[#1C1B18] mb-2">Web Clipper</h3>
              <p className="text-[#6B6966] leading-relaxed">一键保存网页到 AI 记忆，浏览器扩展让知识自动积累</p>
            </div>
          </div>
        </FadeIn>
      </section>
      ...
      {/* ===== How It Works ===== */}
      <section className="relative z-10 mx-auto max-w-5xl px-6 py-20 bg-[#F8F7F5] rounded-3xl">
        <FadeIn>
          <h2 className="text-center text-3xl font-bold text-[#1C1B18] sm:text-4xl">看看 AI Drive 如何工作</h2>
          <div className="mt-16 grid gap-8 sm:grid-cols-3">
            {STEPS.map((s, i) => (
              <div key={s.title} className="relative flex flex-col items-center text-center">
                {/* connector line */}
                {i < STEPS.length - 1 && (
                  <ChevronRight className="absolute -right-5 top-8 hidden h-6 w-6 text-[#E5E4E1] sm:block" />
                )}
                ...
                <h3 className="mt-2 text-lg font-semibold text-[#1C1B18]">{s.title}</h3>
          ...
          <FadeIn key={f.title}>
            <div className={`flex flex-col items-center gap-12 md:flex-row ${f.reverse ? "md:flex-row-reverse" : ""}`}>
              ...
              <div className="flex-1 space-y-4">
                ...
                <h3 className="text-2xl font-bold text-[#1C1B18] sm:text-3xl">{f.title}</h3>
      ...
      <section className="relative z-10 px-6 py-32 text-center">
        <FadeIn>
          <h2 className="text-3xl font-bold text-[#1C1B18] sm:text-4xl">准备好了吗？让 AI 记住你的一切</h2>
          <div className="mt-8">
