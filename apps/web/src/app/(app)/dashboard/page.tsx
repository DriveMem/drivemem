            ...
            <div className="mx-6 mb-4 rounded-xl border border-amber-200 bg-amber-50/50 dark:bg-amber-950/20 dark:border-amber-800 p-4">
              ...
              <div className="space-y-2">
                {conflicts.slice(0, 5).map((c: any) => (
                  <div key={c.id} className="flex items-center gap-2 text-xs rounded-lg border border-amber-200/60 dark:border-amber-800/40 bg-white/50 dark:bg-zinc-900/50 px-3 py-2">
                    <span className="shrink-0">⚡</span>
                    <div className="flex-1 min-w-0">
                      <span className="text-amber-700 dark:text-amber-300">
                        <a href={`/files/${c.sourceFile?.id || c.sourceFileId}/preview`} className="font-medium hover:underline">{c.sourceFile?.name || "File"}</a>
                        {" "}contradicts{" "}
                        <a href={`/files/${c.targetFile?.id || c.relatedFileId}/preview`} className="font-medium hover:underline">{c.targetFile?.name || "File"}</a>
                      </span>
                      {c.description && <p className="text-amber-600/70 dark:text-amber-400/70 mt-0.5 line-clamp-1">{c.description}</p>}
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                      <a href={`/files/${c.sourceFile?.id || c.sourceFileId}/preview`} className="p-1 rounded hover:bg-amber-100 dark:hover:bg-amber-900/30 transition-colors" title="View source">
                        <FileText className="h-3.5 w-3.5 text-amber-500" />
                      </a>
                      <a href={`/files/${c.targetFile?.id || c.relatedFileId}/preview`} className="p-1 rounded hover:bg-amber-100 dark:hover:bg-amber-900/30 transition-colors" title="View target">
                        <FileText className="h-3.5 w-3.5 text-amber-500" />
                      </a>
                    </div>
                  </div>
                ...
                {conflicts.length > 5 && (
                  <p className="text-xs text-amber-500 pl-2">+ {conflicts.length - 5} more conflicts</p>
                )}
              </div>
            </div>
          )}
