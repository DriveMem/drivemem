...
import { embedTexts, chatStream } from '../services/llm.service.js';
import { CHAT_CONTEXT_ROUNDS } from '@ai-drive/shared';
...
export default async function conversationRoutes(app: FastifyInstance) {
  ...
  // DELETE /:id — delete conversation (cascades messages)
  app.delete('/:id', { preHandler: [requireAuth] }, async (request, reply) => {
    ...
    // Messages cascade via FK, but explicit delete for safety
  ...
  app.post('/:id/messages', { preHandler: [requireAuth] }, async (request, reply) => {
    ...
    // --- Daily chat limit check (BE-WS5-05) ---
    const [user] = await db.select().from(users).where(eq(users.id, userId));
    const now = new Date();
    const todayUTC = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));

    if (!user.lastChatResetAt || user.lastChatResetAt < todayUTC) {
      // Reset daily count
      await db
        .update(users)
    ...
    if (user.dailyChatCount >= user.dailyChatLimit) {
    ...
    // Get recent context (last N rounds)
    const contextRounds = CHAT_CONTEXT_ROUNDS ?? 5;
...
${citationSources.join('\n\n')}`;
    ...
    try {
      const stream = await chatStream(systemPrompt, chatHistory);
