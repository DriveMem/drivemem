...
function humanizeFileName(name: string, summary?: string | null): string {
  // session-summary-*.md → use summary or "Session Summary"
  if (/^session-summary-/i.test(name)) {
    return summary?.slice(0, 80) || 'Session Summary';
  }
  // auto-capture-*.md → use summary or clean name
  if (/^auto-capture-/i.test(name)) {
    return summary?.slice(0, 80) || name.replace(/^auto-capture-/, '').replace(/\.\w+$/, '').replace(/[-_]/g, ' ');
  }
  // auto-*.md (other auto patterns)
  if (/^auto-/i.test(name)) {
    return summary?.slice(0, 80) || name.replace(/^auto-/, '').replace(/\.\w+$/, '').replace(/[-_]/g, ' ') || 'Auto Note';
  }
  // note-YYYY-MM-DDTHH-MM-SS*.md → use summary or "Note"
  if (/^note-\d{4}-\d{2}-\d{2}T/i.test(name)) {
    return summary?.slice(0, 80) || 'Note';
  }
  return name;
}
...
export async function fetchTimeline(userId: string, limit: number, cursor?: string) {
  ...
  // Cross-reference: find which files were stored by agents (via activity logs)
  ...
  if (fileIds.length > 0) {
    const storeActivities = await db.select({
      ...
      .limit(200);
    // Match store activities to files by checking metadata.fileId or detail matching file name
    for (const sa of storeActivities) {
  ...
  const events: TimelineEvent[] = [
    ...files.map(f => {
      ...
      const isAutoGenerated = isAutoCapture || /^(session-summary-|auto-|note-\d{4}-\d{2}-\d{2}T)/i.test(f.name);
      // Humanize all auto-generated filenames using summary when available
      const displayTitle = isAutoGenerated
        ? humanizeFileName(f.name, f.summary)
        : f.name;
      const storedByAgent = fileAgentMap[f.id];
      return {
      id: f.id,
      type: (isAutoCapture ? 'auto_capture' : 'file_uploaded') as 'auto_capture' | 'file_uploaded',
      title: displayTitle,
      description: isAutoGenerated ? undefined : (f.summary?.slice(0, 100) || undefined),
      icon: isAutoCapture ? '🧲' : '📄',
...
export default async function timelineRoutes(fastify: FastifyInstance) {
  fastify.get('/', { preHandler: [requireAuth] }, async (request, reply) => {
    ...
    const typeFilter = query.type || undefined; // 'files' | 'conversations' | 'agent' | undefined (all)
    ...
    if (typeFilter) {
      const typeMap: Record<string, string[]> = {
        ...
        agent: ['agent_activity', 'auto_capture', 'relay'],
      }
      const allowedTypes = typeMap[typeFilter]
      if (allowedTypes) {
        result.events = result.events.filter(e => allowedTypes.includes(e.type))
      }
    }
